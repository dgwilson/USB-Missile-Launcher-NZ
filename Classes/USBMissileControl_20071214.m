...
//	missileLauncherConnected = [self FindMissileLauncher];
    ...
    if (kr || !masterPort)
    {
		NSLog(@"USBMissileControl: could not create master port, err = %08x", err);
        return NO;
    }
...
void DeviceAdded(void *refCon, io_iterator_t iterator)
{
    ...
    while ((usbDevice = IOIteratorNext(iterator)) != 0)
    {
		...
		CFNumberGetValue(IORegistryEntryCreateCFProperty(usbDevice, CFSTR(kUSBVendorID), kCFAllocatorDefault, 0), kCFNumberSInt32Type, &usbVendorID);
		CFNumberGetValue(IORegistryEntryCreateCFProperty(usbDevice, CFSTR(kUSBProductID), kCFAllocatorDefault, 0), kCFNumberSInt32Type, &usbProductID);
		...
		NSLog(@"USBMissileControl: DeviceAdded: usbVendorID: %d(0x%x) usbProductID: %d(0x%x) : %@", usbVendorID, usbVendorID, usbProductID, usbProductID, [privateDataRef getLauncherType]);
...
//			NSLog(@"USBMissileControl: DeviceAdded: success - IOObjectRelease(usbDevice)");
		...
		kr = FindInterfaces(deviceInterface);
        if (KERN_SUCCESS == kr)
        {
//			NSLog(@"USBMissileControl: DeviceAdded: success - FindInterfaces(deviceInterface)");
		...
		*/
        ...
        if (KERN_SUCCESS == kr)
        {
			NSLog(@"USBMissileControl: DeviceAdded: success - reading HIDDescriptor");
		}
                         ...
                         void *messageArgument )
{
	...
	//NSLog(@"USBMissileControl: DeviceNotification: REMOVAL Notification: 0x%08x", service);
    if (messageType == kIOMessageServiceIsTerminated)
    {
		...
		{
			NSLog(@"USBMissileControl: DeviceNotification: %d index", i);
	...
	//		kIOMessageSystemWillPowerOn
		...
		if (messageType == kIOMessageCanDevicePowerOff)
		{
		...
		if (messageType == kIOMessageSystemWillNotPowerOff)
		{
		...
		if (messageType == kIOMessageSystemWillNotSleep)
		{
		...
		if (messageType == kIOMessageSystemWillRestart)
		{
...
//			[privateDataRef getusbProductID] == kUSBMissileProductID)
		if ([[privateDataRef getLauncherType] isEqualToString:@"StrikerII"])
		{
			...
			if (debugCommands)
			{
				...
				NSLog(@"USBMissileControl: USBVendorID  = %d (0x%x)", [privateDataRef getusbVendorID], [privateDataRef getusbVendorID]);
				NSLog(@"USBMissileControl: USBProductID = %d (0x%x)", [privateDataRef getusbProductID], [privateDataRef getusbProductID]);
			...
			if (controlBits & 16)
			{
				...
				for (delayCounter = 0; delayCounter < 70; delayCounter ++)
				{
					...
					if (rBuffer[1] >= 0x80)
					{
						...
						[NSThread sleepUntilDate:[[NSDate alloc]initWithTimeIntervalSinceNow:1.000]];
						...
						if (debugCommands)
						{
							...
							NSLog(@"  devRequest.wIndex        %x", devRequest.wIndex);
							NSLog(@"  devRequest.wLength       %x", devRequest.wLength);
							NSLog(@"USBMissileControl: DreamCheeky command package (0x%02x) (0x%02x) (0x%02x) (0x%02x) (0x%02x) (0x%02x) (0x%02x)", reqBuffer[0], reqBuffer[1], reqBuffer[2], reqBuffer[3], reqBuffer[4], reqBuffer[5], reqBuffer[6], reqBuffer[7]);
						}
						kr = (*missileDevice)->DeviceRequest(missileDevice, &devRequest);
						if (debugCommands)
						{
...
//				[privateDataRef getusbProductID] == kUSBRocketProductID)
...

		{
			...
			// refer e-mail from Josiah Greaves
			// ===========================================================================
			if (debugCommands)
			{
				NSLog(@"USBMissileControl: launcherType = %@", [privateDataRef getLauncherType]);
			...
			// Control of the launcher works on a binary code - see the table below for an explanation
			//
			// Set up Packet - 21 09 00 02 00 00 00 00 (Assumption)
			//
			// 01  - down
			// 02  - up
			// 04  - left
			...
			//     | Fire |RT |LT |UP |DN |
			//
			//		Thanks to Brandon Heyer for the following: *** TBC for this version of the launcher
			//      the DreamCheeky Launcher will return the following codes
			//	
			//		00 04 00 00 00 00 00 00 - All the way left
			//		00 08 00 00 00 00 00 00 - All the way right
			//		40 00 00 00 00 00 00 00 - All the way down
			//		80 00 00 00 00 00 00 00 - All the way up 
			//		10 00 00 00 00 00 00 00 - Fire Has completed 
			//		00 00 00 00 00 00 00 00 - Fire Has NOT completed 
			//		00 84 00 00 00 00 00 00 - Fire Has completed and we're all the way left
			//		00 88 00 00 00 00 00 00 - Fire Has completed and we're all the way right
			...
			{	
				if (debugCommands)
					NSLog(@"USBMissileControl: return from RocketBabyReadPipe (0x%08x)", rbBuffer[0]);
			}
...
/*
			// THIS PACKAGE OF NULLS IS NOT REQUIRED TO BE DELIVERED TO THE ROCKET BABY LAUNCHER
			// send the first package - NULL
			reqBuffer[0] = 0;
			reqBuffer[1] = 0;
			reqBuffer[2] = 0;
			...
			if (debugCommands)
			{
				...
				NSLog(@"  devRequest.wIndex        %x", devRequest.wIndex);
				NSLog(@"  devRequest.wLength       %x", devRequest.wLength);
				NSLog(@"USBMissileControl: Rocket Baby command package (0x%02x) (0x%02x) (0x%02x) (0x%02x) (0x%02x) (0x%02x) (0x%02x) (0x%02x)", reqBuffer[0], reqBuffer[1], reqBuffer[2], reqBuffer[3], reqBuffer[4], reqBuffer[5], reqBuffer[6], reqBuffer[7]);
			}
//			kr = (*missileInterface)->DeviceRequest(missileInterface, &devRequest);
			kr = (*missileDevice)->DeviceRequest(missileDevice, &devRequest);
			if (debugCommands) 
			{
				if (kr != kIOReturnSuccess)
 ...
 */
			...
			if (debugCommands)
			{
				...
				NSLog(@"  devRequest.wIndex        %x", devRequest.wIndex);
				NSLog(@"  devRequest.wLength       %x", devRequest.wLength);
				NSLog(@"USBMissileControl: Rocket Baby command package (0x%02x)", reqBuffer_RB[0]);
			}
			kr = (*missileDevice)->DeviceRequest(missileDevice, &devRequest);
			if (debugCommands)
			{
			...
			if (controlBits & 16)
			{
				...
				for (delayCounter = 0; delayCounter < 70; delayCounter ++)
				{
					...
					if (rbBuffer[0] & 0x10)
					{
						...
						[NSThread sleepUntilDate:[[NSDate alloc]initWithTimeIntervalSinceNow:0.500]];
...
IOReturn DreamCheekyReadPipe(IOUSBDeviceInterface **missileDevice, IOUSBInterfaceInterface **missileInterface, UInt8 *rBuffer)
{
	...
	if (debugCommands)
		NSLog(@"USBMissileControl: DreamCheekyReadPipe: ReadPipe -> bytesRead setup=%d", bytesRead);
...
IOReturn RocketBabyReadPipe(IOUSBDeviceInterface **missileDevice, IOUSBInterfaceInterface **missileInterface, UInt8 *rBuffer)
{
	...
	if (debugCommands)
	{
		...
		NSLog(@"  devRequest.wIndex        %x", devRequest.wIndex);
		NSLog(@"  devRequest.wLength       %x", devRequest.wLength);
		NSLog(@"USBMissileControl: Rocket Baby ReadPipe (0x%02x)", reqBuffer[0]);
	}
	kr = (*missileDevice)->DeviceRequest(missileDevice, &devRequest);
	if (debugCommands)
	{
...
//		{
//			if (debugCommands)
//				NSLog(@"USBMissileControl: RocketBabyReadPipe: All the way down");
//		} else 
//			if (rBuffer[0] == 0x80)
//			{	
//				if (debugCommands)
//					NSLog(@"USBMissileControl: RocketBabyReadPipe: All the way up");
//			}
		
//		if ((rBuffer[1] == 0x04) || (rBuffer[1] == 0x84))  // this command response can get mixed up with Fire
//		{	
//			if (debugCommands)
//				NSLog(@"USBMissileControl: RocketBabyReadPipe: All the way left");
//		} else 
//			if ((rBuffer[1] == 0x08) || (rBuffer[1] == 0x88))  // this command response can get mixed up with Fire
//			{
//				if (debugCommands)
//					NSLog(@"USBMissileControl: RocketBabyReadPipe: All the way right");
//			}
...
//	NSLog(@"USBMissileControl: DreamCheekyWritePipe: value of rBuffer bytesRead %x", bytesRead);
	...
	NSLog(@"USBMissileControl: DreamCheekyWritePipe: gBuffer %d, %d, %d -- %d", gBuffer[0], gBuffer[1], gBuffer[2], bytesToWrite);
